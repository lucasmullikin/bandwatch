...
The table lives in your config directory (config/stations.json), not in this
...
def load_sets():
    ...
    if not isinstance(sets, dict) or not sets:
        raise C.ConfigError(
            "stations.json has no 'sets' object.\n"
            "  See config/examples/stations.json for the shape.")
    out = {}
    placeholders = []
    for name, spec in sets.items():
        if not isinstance(spec, dict):
            raise C.ConfigError("stations.json: set %r is not an object" % name)
        ...
        if mode not in ("am", "nfm"):
            raise C.ConfigError(
                "stations.json: set %r has mode %r; expected \"am\" or \"nfm\"" % (name, mode))
        chans = spec.get("channels")
        if not isinstance(chans, list) or not chans:
            raise C.ConfigError("stations.json: set %r has no channels" % name)
        pairs = []
        for ch in chans:
            if not (isinstance(ch, list) and len(ch) == 2):
                raise C.ConfigError(
                    "stations.json: set %r has a channel that is not [label, mhz]: %r" % (name, ch))
            label, freq = ch
            if not isinstance(freq, (int, float)):
                raise C.ConfigError(
                    "stations.json: set %r channel %r has a non-numeric frequency %r"
                    % (name, label, freq))
    ...
    if placeholders and "--allow-placeholders" not in sys.argv:
        raise C.ConfigError(
            ...
            % (", ".join(sorted(placeholders)), C.path("stations.json")))
    return out
...
# {note}
#
# GENERATED by make_confs.py from config/stations.json. Do not hand-edit:
